...
int main()
{
    ...
    for (k = 0; k < 50; k++)
    {
        ...
        for (j = 0; j < 500; j++)
        {
            ...
            for (i = 0; i < 256; i++)
                _mm_clflush((const void *)(attacker_array + i * Cache_Line));
            ...
            for (i = 0; i < 6; i++)
            {
                ...
                // if (attacker_index < victim_array_size)
                //     temp = attacker_array[victim_array[attacker_index] * Cache_Line];
                // 隔离
                _mm_mfence();
            }
            ...
            for (i = 0; i < 256; i++)
                if (difference[i] < HIT_THRESHHOLD)
                    probability_count[i]++;
